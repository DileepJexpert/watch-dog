# SENTINEL AI-Enhanced Architecture
...
SENTINEL is a real-time production-support observability platform with two layers:
...
| **NFR** | Dual Run Mode | IDE + infra Docker (`demo/docker-compose.infra-only.yml`) OR full Docker pipeline (`sentinel-backend/docker-compose.yml`). CI validates both. | `sentinel-backend/docker-compose.yml`, `demo/docker-compose.infra-only.yml`, `.github/workflows/ci.yml` |
...
SENTINEL ships with a native `OllamaLlmClient` so you can talk to Ollama directly
...
Set the env vars in `sentinel-backend/docker-compose.yml` or pass them at deploy time:
...
└── config/                   # Spring configuration
    ├── SentinelProperties.java   # All config as typed Java
...
```
sentinel-backend/docker-compose.yml   # Full-stack (CI/pipeline + IntelliJ one-click)
