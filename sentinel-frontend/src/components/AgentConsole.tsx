...
export const AgentConsole: React.FC = () => {
  ...
  const refreshStatus = () => {
    ...
    axios.get<AgentStatus>(`${SENTINEL_API}/api/agent/status`)
  ...
  const refreshModels = () => {
    ...
    axios.get<ModelsResponse>(`${SENTINEL_API}/api/agent/models`)
  ...
  const onModelChange = async (newModel: string) => {
    ...
    try {
      const res = await axios.post(`${SENTINEL_API}/api/agent/model`, { model: newModel });
  ...
  const send = async () => {
    ...
    if (tryStream) {
      try {
        ...
        await axios.post(`${SENTINEL_API}/api/agent/ask/stream`, {
    ...
    try {
      ...
      const res = await axios.post<AgentAnswer>(`${SENTINEL_API}/api/agent/ask`, {
        question, history, sessionId
      });
